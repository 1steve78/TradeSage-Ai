...
import { newsPrompt } from "../services/ai/promptService.js";
...
export const NewsStateAnnotation = Annotation.Root({
  ...
  type: Annotation({ reducer: (x, y) => (y !== undefined ? y : x), default: () => "NEWS" }),
...
const cacheLookupNode = async (state) => {
  ...
  const cached = await findCachedInsight(state.contextHash, "NEWS");
...
const promptBuilderNode = async (state) => {
  const prompt = newsPrompt(state.context);
  return { prompt };
};
