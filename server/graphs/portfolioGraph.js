...
import { portfolioSummaryPrompt } from "../services/ai/promptService.js";
...
export const PortfolioStateAnnotation = Annotation.Root({
  ...
  type: Annotation({ reducer: (x, y) => (y !== undefined ? y : x), default: () => "PORTFOLIO_SUMMARY" }),
...
const cacheLookupNode = async (state) => {
  ...
  const cached = await findCachedInsight(state.contextHash, "PORTFOLIO_SUMMARY");
...
const promptBuilderNode = async (state) => {
  const prompt = portfolioSummaryPrompt(state.context);
  return { prompt };
};
...
const workflow = new StateGraph(PortfolioStateAnnotation)
