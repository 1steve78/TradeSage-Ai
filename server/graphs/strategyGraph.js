...
export const StrategyStateAnnotation = Annotation.Root({
  ...
  type: Annotation({ reducer: (x, y) => (y !== undefined ? y : x), default: () => "STRATEGY" }),
...
const cacheLookupNode = async (state) => {
  ...
  const cached = await findCachedInsight(state.contextHash, "STRATEGY");
...
const workflow = new StateGraph(StrategyStateAnnotation)
...
export const strategyGraph = workflow.compile();
