 ...
 */
export const getMarketPulse = catchAsync(async (req, res) => {
  ...
  if (!result || !result.response) {
    throw new AppError("Unable to generate market insights.", 500, "AI_GENERATION_FAILED");
  }
 ...
 */
export const getInsightHistory = catchAsync(async (req, res) => {
  const userId = req.user?._id || req.user?.id || null;
 ...
 */
export const getAIStats = catchAsync(async (req, res) => {
  const userId = req.user?._id || req.user?.id || null;
  const query = userId ? { userId } : {};
